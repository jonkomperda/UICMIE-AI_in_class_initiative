function Neural_network_derivative_demo
    ...
    results_table = makeResultsTable(x, y, dydx);
...
% numericalDerivatives Compute first derivatives using second-order accurate
...
function results_table = makeResultsTable(x, y, dydx)
% makeResultsTable Assemble x, y, and derivative data into a MATLAB table.

    results_table = table(x(:), y(:), dydx(:), ...
        'VariableNames', {'x', 'y', 'y_prime'});
end

function makePlots(x, y, dydx)
% makePlots Plot the function and its numerical derivative in subplots.
